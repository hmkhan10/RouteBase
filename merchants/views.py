...
from .models import (
    ...
    Order, Refund, Payout, WebhookConfiguration, WebhookLog
)
...
def build_page(request):
    if request.method == "POST":
        ...
        try:
            ...
            # 4. Generate Unique ID (8-digit code)
            unique_id = str(uuid.uuid4())[:8].upper()
            sub.paymob_order_id = unique_id  # Storing ID here for now
            sub.is_active = True
            sub.save()

            # 5. Show Success Page with ID and URL
            context = {
                'unique_id': unique_id,
...
# Professional Payment Platform APIs
@api_view(['POST'])
...
def create_refund(request):
    ...
    try:
        ...
        if not all([order_id, amount, reason]):
            return JsonResponse({
                'error': 'Missing required fields: order_id, amount, reason'
        ...
        # Validate amount
        if Decimal(str(amount)) > order.amount:
            return JsonResponse({
        ...
        with transaction.atomic():
            refund = Refund.objects.create(
                ...
                processor=data.get('processor', 'routebase'),
...
def create_payout(request):
    ...
    try:
        ...
        if not all([amount, payout_method]):
            return JsonResponse({
                'error': 'Missing required fields: amount, payout_method'
            }, status=400)
        
        # Validate minimum payout amount (PKR 1000)
        if Decimal(str(amount)) < Decimal('1000.00'):
            return JsonResponse({
                'error': 'Minimum payout amount is PKR 1000'
            }, status=400)
        
        # Calculate fees (3% platform fee)
        gross_amount = Decimal(str(amount))
        platform_fee = gross_amount * Decimal('0.03')
        tax_withheld = gross_amount * Decimal('0.05')  # 5% tax
        net_amount = gross_amount - platform_fee - tax_withheld
...
def configure_webhook(request):
    """Configure webhook for the authenticated user"""
    try:
        data = json.loads(request.body)
        ...
        webhook_secret = data.get('webhook_secret') or f"wh_{uuid.uuid4().hex[:32]}"
        
        # Create or update webhook configuration
        with transaction.atomic():
            config, created = WebhookConfiguration.objects.update_or_create(
                merchant=request.user,
                defaults={
            ...
            if not created:
                ...
                config.allowed_ips = data.get('allowed_ips', [])
...
def webhook_receiver(request, webhook_secret):
    ...
    try:
        ...
        config = WebhookConfiguration.objects.filter(webhook_secret=webhook_secret).first()
        ...
        if response.status_code != 200 and webhook_log.attempt_count < webhook_log.max_attempts:
...
def trigger_webhook(request):
    ...
    try:
        ...
        config = WebhookConfiguration.objects.filter(merchant=request.user).first()
        if not config:
            return JsonResponse({
        ...
        return JsonResponse({
            'success': True,
            'message': f'Webhook {event_type} triggered successfully',
            'webhook_url': config.webhook_url
        })
...
def webhook_logs(request):
    """Get webhook delivery logs for the authenticated user"""
    try:
        config = WebhookConfiguration.objects.filter(merchant=request.user).first()
        if not config:
            return JsonResponse({
        ...
        logs = WebhookLog.objects.filter(config=config).order_by('-created_at')[:50]  # Last 50 logs
